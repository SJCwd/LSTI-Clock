...
DATA_DIR = {
    YELP_STAR: '/data/user/zjh/recommend/processData/SCPR-data/data/yelp',
    MOVIE: '/data/user/zjh/recommend/processData/movie-20M_dict/new',
}
TMP_DIR = {
...
def load_rl_agent(dataset, filename, epoch_user):
    ...
    model_file = SAVE_TMP_DIR_DISK1[dataset] + '/RL-agent/' + filename + '-epoch-{}.pkl'.format(epoch_user)
...
def save_rl_agent(dataset, model, filename, epoch_user):
    # model_file = TMP_DIR[dataset] + '/RL-agent/' + filename + '-epoch-{}.pkl'.format(epoch_user)   ## SAVE_TMP_DIR_DISK0
    model_file = SAVE_TMP_DIR_DISK1[dataset] + '/RL-agent/' + filename + '-epoch-{}.pkl'.format(epoch_user)
...
def date2array(date_string, date_name='MOVIE'):
    ...
    if date_name in ('YELP_STAR',):
        date_object = datetime.strptime(date_string, "%Y-%m-%d").timetuple()  # 返回日期对应的time.struct_time对象
        # （tm_year, tm_mon, tm_mday, tm_hour, tm_min, tm_sec, tm_wday, tm_yday, tm_isdst）
        year = date_object.tm_year
        month = date_object.tm_mon  # 范围 [1, 12]
        day = date_object.tm_mday  # 范围 [1, 31]
        weekday = date_object.tm_wday  # 范围 [0, 6]，星期一为 0
        # hour = date_object.tm_hour  # 范围 [0, 23]
        ...
        date_object = datetime.strptime(date_string, "%Y-%m-%d %H:%M:%S").timetuple()  # 返回日期对应的time.struct_time对象
        # （tm_year, tm_mon, tm_mday, tm_hour, tm_min, tm_sec, tm_wday, tm_yday, tm_isdst）
        year = date_object.tm_year
        month = date_object.tm_mon  # 范围 [1, 12]
        day = date_object.tm_mday  # 范围 [1, 31]

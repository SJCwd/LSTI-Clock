...
from RL.env_timeAware_binary_question_woTimeFusion import NoTimeFusioneBinaryRecommendEnv
from utils import *
...
noTime_EnvDict = {
    MOVIE: NoTimeFusioneBinaryRecommendEnv,
    YELP_STAR: NoTimeFusioneBinaryRecommendEnv
}

SAVE_TMP_DIR_DISK1 = {
    YELP_STAR: '/data1/zjh/crs_work2_concate_addTurnPunish/save_model/yelp_star',
    MOVIE:'/data1/zjh/crs_work2_concate_addTurnPunish/save_model/movie'
}
...
RL_model_YELP_STAR_2690_test_value_all = {}
RL_model_YELP_STAR_2690_test_value_sr15 = {}
best_sr15, best_avgt, best_epoch = -1, 16, -1
...
def train(args, kg, dataset, filename, test_epoch_list = None):
    global best_sr15, best_avgt, best_epoch
    global RL_model_YELP_STAR_2690_test_value_all,RL_model_YELP_STAR_2690_test_value_sr15

    if args.no_fusion:
        env = noTime_EnvDict[args.data_name](kg, dataset, args.data_name, args.embed, seed=args.seed, max_turn=args.max_turn,
                                      cand_num=args.cand_num, cand_item_num=args.cand_item_num,
                                      attr_num=args.attr_num, mode='train', ask_num=args.ask_num,
                                      entropy_way=args.entropy_method, fm_epoch=args.fm_epoch, args=args)
    else:
    ...
    time_to_hidden = torch.FloatTensor(env.time_to_hidden)  # 将16维拼起来后再转为hidden dim
    gcn_net = GraphEncoder(device=args.device, entity=embed.size(0), emb_size=embed.size(1), kg=kg, embeddings=embed, \
                           ...
                           seq_conv_name= args.seq_conv_name, seq_conv_layer=args.seq_conv_layer,
                           time_to_hidden = time_to_hidden
                           ).to(args.device)
    ...
    if args.mode == 'test':
        print('start test ...')
        if test_epoch_list == None:
            test_epoch_list = {args.load_rl_epoch: 1000}
        for load_rl_epoch in test_epoch_list:
            file_path = SAVE_TMP_DIR_DISK1[args.data_name] + '/RL-agent/' + filename + '-epoch-{}.pkl'.format(load_rl_epoch)
            if not os.path.exists(file_path):
                print(f'{load_rl_epoch} 不存在，跳过该轮')
                continue
            args.load_rl_epoch = load_rl_epoch
            print(f'\t test_load_epoch is {args.load_rl_epoch}: {test_epoch_list[args.load_rl_epoch]}')
            print('Staring loading rl model in epoch {}'.format(args.load_rl_epoch))
            agent.load_model(data_name=args.data_name, filename=filename, epoch_user=args.load_rl_epoch)
            ...
            print(
                f'load_rl_epoch = {args.load_rl_epoch}; SR5_mean = {SR5_mean}; SR10_mean = {SR10_mean}; SR15_mean = {SR15_mean}; AvgT_mean = {AvgT_mean}; Rank_mean = {Rank_mean}')
            print('************************************************************')

            #记录当前结果
            RL_model_YELP_STAR_2690_test_value_all[args.load_rl_epoch] = f'SR5_mean = {SR5_mean}; SR10_mean = {SR10_mean}; SR15_mean = {SR15_mean}; AvgT_mean = {AvgT_mean}; Rank_mean = {Rank_mean}'
            RL_model_YELP_STAR_2690_test_value_sr15[args.load_rl_epoch] = SR15_mean

            if (SR15_mean > best_sr15) or (SR15_mean == best_sr15 and AvgT_mean < best_avgt):
                #最优交换
                best_sr15, best_avgt, best_epoch = SR15_mean, AvgT_mean, args.load_rl_epoch
            print(f'最优epoch is {best_epoch}')
            print(f'{RL_model_YELP_STAR_2690_test_value_all[best_epoch]}')
        return
    #####################################
    test_performance = []
    if args.eval_num == 1:
    ...
    for train_step in range(args.load_rl_epoch + 1, args.max_steps+1):
        ...
        for i_episode in tqdm(range(args.sample_times),desc='sampling'):
            ...
            for t in count():   # user  dialog
                ...
                # cost_times[t] = cost_times[t] + (endtime-start_time)
        ...
        # print(f'cost_times ： {cost_times}')
        # averge_time = [tuen_time / 100.0 for tuen_time in cost_times]
        # print(f'averge cost_times ： {averge_time}')
        ...
        #####################################################  记录了所有bar大于要求的
        if (SR15 / args.sample_times) >= sr15_bar_dict[args.data_name]:
            train_all_ok_epoch[train_step] = SR15 / args.sample_times
        #####################################################

        # if train_step % args.eval_num == 0:
        #     SR15_mean = dqn_evaluate(args, kg, dataset, agent, filename, train_step)
        #     test_performance.append(SR15_mean)
        if train_step % args.save_num == 0 and ((SR15 / args.sample_times) >= sr15_bar_dict[args.data_name]):
    ...
    print('*********************开始寻找最优*********************')
    print(train_all_ok_epoch)
    # best_SR5, best_SR10, best_SR15, best_AvgT, best_Rank, best_epoch = SR5_mean, SR10_mean, SR15_mean, AvgT_mean, Rank_mean, train_step
    # #########################################################################
    # for test_step in reversed(train_all_ok_epoch):
    #     if test_step == args.max_steps:
    #         continue
    #     print(f'现在的epoch 是 {test_step}')
    #     agent.load_model(data_name=args.data_name, filename=filename, epoch_user=test_step)
    #     SR5_mean, SR10_mean, SR15_mean, AvgT_mean, Rank_mean = dqn_evaluate(args, kg, dataset, agent, filename,test_step)
    #     if (SR15_mean > best_SR15) or (SR15_mean == best_SR15 and AvgT_mean < best_AvgT):
    #         best_SR5, best_SR10, best_SR15, best_AvgT, best_Rank, best_epoch = SR5_mean, SR10_mean, SR15_mean, AvgT_mean, Rank_mean, test_step
    #         print('best changed!')
    #     print(f'best_SR5={best_SR5}, best_SR10={best_SR10}, best_SR15={best_SR15}, best_AvgT={best_AvgT}, best_Rank={best_Rank}, best_epoch={best_epoch}')
    # #########################################################################
...
def main():
    ...
    ######################################################################################
    parser.add_argument('--no_fusion', action='store_true', help='env user-time not fusion ')
    ######################################################################################

    parser.add_argument('--seq_conv_name', type=str, default='gcn')
    parser.add_argument('--seq_conv_layer', type=int, default=1)
    ...
    # filename = 'TransAll-fusionTime-train-data-{}-RL-cand_num-{}-cand_item_num-{}-embed-{}-seq-{}-gcn-{}-conv_name-{}-gnnLayer-{}-time_emb_file-{}'.format(
    ...
    filename = 'TrainParam-SIGMOD-Trans-Concate-addTurnPunish-train-data-{}-RL-cand_num-{}-cand_item_num-{}-embed-{}-seq-{}-gcn-{}-conv_name-{}-gnnLayer-{}-time_emb_file-{}-seq_conv_layer-{}'.format(
        args.data_name, args.cand_num, args.cand_item_num, args.embed, args.seq, args.gcn, args.conv_name, args.gnn_layers, args.time_emb_file, args.seq_conv_layer)
    # if args.no_fusion:
    #     filename = 'TransPart-nofusion-train-data-{}-RL-cand_num-{}-cand_item_num-{}-embed-{}-seq-{}-gcn-{}-conv_name-{}-gnnLayer-{}-time_emb_file-{}'.format(
    #         args.data_name, args.cand_num, args.cand_item_num, args.embed, args.seq, args.gcn, args.conv_name,
    #         args.gnn_layers, args.time_emb_file)

    print(f'filename = {filename}')

    if args.mode == 'test':
        test_epoch = {100:1}
        train(args, kg, dataset, filename, test_epoch_list=test_epoch)
    else:
        print('train mode...')
        train(args, kg, dataset, filename)

    print('**************************all is over**************************')
    print(f'best_epoch = {best_epoch}; best_sr15 = {best_sr15}; best_avgT = {best_avgt}')
    print(f'{RL_model_YELP_STAR_2690_test_value_sr15}')
    print(f'{RL_model_YELP_STAR_2690_test_value_all}')
    print('**************************all is over**************************')

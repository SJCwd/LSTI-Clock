...
def dqn_evaluate(args, kg, dataset, agent, filename, i_episode):
    ...
    if args.data_name in [YELP_STAR, MOVIE]:
        if args.eval_num == 1:
            ...
            # test_size = 539     # Only do 1000 iteration for the sake of time   case study找2000个   2.14的只有539个
            test_size = 1000
        user_size = test_size
    print('The select Test size : ', test_size)

    ############################ for case study
    # success_case_dict = {}  # 记录成功的
    ############################ for case study
        ...
        for t in count():  # user  dialog
            ...
            if done:
                ...
                if reward.item() == 1:  # recommend successfully
                    ## for case study
                    # success_case_dict[test_env.user_id] = {}
                    # success_case_dict[test_env.user_id]['target_item'] = test_env.target_item
                    # success_case_dict[test_env.user_id]['target_item_date'] = test_env.target_item_date
                    # success_case_dict[test_env.user_id]['turn'] = t
    ...
    ############################ for case study
    # savePKL(f'./{args.data_name}_success_case_dict_{user_size}_214day.pkl',success_case_dict)
    ############################ for case study

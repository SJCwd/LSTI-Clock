...
class TimeAwareBinaryRecommendEnv(object):
    def __init__(self, kg, dataset, data_name, embed, seed=1, max_turn=15, cand_num=10, cand_item_num=10, attr_num=20,
        ...
        self.ui_dict = self.__load_rl_data__(data_name, mode=mode)  # 加载数据 就是简单的json.load
        ...
        if mode == 'train':
            ...
            self.__test_tuple_generate__()      ## ui_list.append([user_id, item_id, interact_date, seq])  其中seq是一个[[item,time],[item,time],... ] 代表交互过的item序列
            self.test_num = 0
    ...
    def __load_rl_data__(self, data_name, mode):
        if mode == 'train':
            with open(os.path.join(DATA_DIR[data_name], 'UI_Interaction_data/with_seq/review_dict_valid.json'),
                      encoding='utf-8') as f:
                print('train_data: load RL valid data')
                mydict = json.load(f)
        elif mode == 'test':
            with open(os.path.join(DATA_DIR[data_name], 'UI_Interaction_data/with_seq/review_dict_test.json'),
                      encoding='utf-8') as f:
    ...
    def __test_tuple_generate__(self):
        ...
        for user_str, items in self.ui_dict.items():
            ...
            for item_info in items:
                ...
                seq = item_info['seq']
                ui_list.append([user_id, item_id, interact_date, seq])
                # if(is_valentine(interact_date)):
                #     ui_list.append([user_id, item_id, interact_date])
        self.ui_array = np.array(ui_list)
        print(f"valentine test size is {self.ui_array.shape}")
    ...
    def reset(self, embed=None):
        ...
        if self.mode == 'train':
            ...
            while (len(self.ui_dict[str(self.user_id)]) == 0):
                self.user_id = np.random.choice(users)
            ...
            self.seq = target_item_info['seq']  # 序列item特征  [ [item,date],[item,date],.... ]
            self.seq_short_item_date = [ date2array(x[1], date_name=self.data_name) for x in self.seq ]
            self.seq_time_node = [ self._map_to_all_id(x_list,old_type='time') for x_list in self.seq_short_item_date ]   # full_id

        elif self.mode == 'test':
            #################   for case study   ########################
            # self.user_id = 4158  # int
            # self.target_item = 17230  # int
            # self.target_item_date = '2018-02-14'  # str
            #########################################
            self.user_id = int(self.ui_array[self.test_num, 0])   #int
            self.target_item = int(self.ui_array[self.test_num, 1])  #int
            ...
            self.time_node = self._map_to_all_id(self.short_target_item_date, old_type='time')  # full_id
            self.seq = self.ui_array[self.test_num, 3]  # list  item历史序列  [ [item,date],[item,date],.... ]
            self.seq_short_item_date = [date2array(x[1], date_name=self.data_name) for x in self.seq]
            self.seq_time_node = [self._map_to_all_id(x_list, old_type='time') for x_list in
                                  self.seq_short_item_date]  # full_id

            self.test_num += 1

        '''
        构建seq图   这个图是在回合进行中不变化的 ##########################################################
        '''
        self.seq_item_node = [x[0] + self.user_length for x in self.seq]                         # items full_id
        seq_item_node_set = set(self.seq_item_node)
        self.seq_item_node_full_id = list(seq_item_node_set)                                     # 去重 items full_id
        self.seq_item_node_full_id_len = len(self.seq_item_node_full_id)

        seq_fea_node_mul = set()
        for x in self.seq:
            for short_fea in self.kg.G['item'][x[0]]['belong_to']:
                seq_fea_node_mul.add(short_fea + self.user_length + self.item_length)
        self.seq_fea_node = list(seq_fea_node_mul)                                              # 去重  features full_id
        self.seq_fea_node_len = len(self.seq_fea_node)

        ##  self.seq_time_node 是 [[],[],...,[]] 形式                                            # 为full_id        年 月 日 (时) week
        # 使用列表推导式将每个元素由列表转换为元组   ---->    保证可以hash
        tuple_seq_item_time_list_mul = [tuple(sublist) for sublist in self.seq_time_node]

        # 使用集合和元组转换来去重  元组是不可哈希的，所以直接使用集合无法去重
        self.tuple_seq_item_time_list_node = list(
            set(tuple(seq_item_date) for seq_item_date in tuple_seq_item_time_list_mul))

        # 前面key的编码是为了放大, 防止哈希冲突
        self.tuple_seq_item_time_list_node_map = { index + 2*self.user_length + 2*self.item_length + self.feature_length : tuple_times for index, tuple_times in enumerate(self.tuple_seq_item_time_list_node) }
        self.tuple_seq_item_time_list_node_reverse_map = { v:k  for k,v in self.tuple_seq_item_time_list_node_map.items() }
        self.seq_item_time_node_full_id = list(self.tuple_seq_item_time_list_node_map.keys())   # 去重  times full_id
        # print(f'seq_item_time_node_full_id is {self.seq_item_time_node_full_id}')
        # 全部为 full_id
        self.seq_neighbors = self.seq_item_node_full_id + self.seq_fea_node + self.seq_item_time_node_full_id

        seq_graph_idx = dict(enumerate(self.seq_neighbors))
        seq_graph_idx = {v: k for k, v in seq_graph_idx.items()}

        seq_i = []  # 构件图   [ [node1,node2] , [node2,node1] , .... ]   无向图
        # 1. seq_item 之间构建双向边
        for item_index in range(len(self.seq_item_node) - 1):
            seq_i.append([ seq_graph_idx[self.seq_item_node[item_index]],   seq_graph_idx[self.seq_item_node[item_index + 1]] ])
            seq_i.append([seq_graph_idx[self.seq_item_node[item_index + 1]],seq_graph_idx[self.seq_item_node[item_index]]])

        # 2. seq_item 与 fea
        for item in self.seq_item_node:
            for belong_fea in self.kg.G['item'][item - self.user_length]['belong_to']:
                full_id_belong_fea = belong_fea + self.user_length + self.item_length
                seq_i.append([seq_graph_idx[item], seq_graph_idx[full_id_belong_fea]])
                seq_i.append([seq_graph_idx[full_id_belong_fea], seq_graph_idx[item]])

        # 3. seq_item 与 time
        for item, time in self.seq:
            time = date2array(time, date_name=self.data_name)  # short_id
            time = self._map_to_all_id(time, old_type='time')  # full_id
            tuple_time = tuple(time)
            seq_i.append([seq_graph_idx[item + self.user_length], seq_graph_idx[self.tuple_seq_item_time_list_node_reverse_map[tuple_time]]])
            seq_i.append([seq_graph_idx[self.tuple_seq_item_time_list_node_reverse_map[tuple_time]], seq_graph_idx[item + self.user_length]])

        self.seq_i = torch.LongTensor(seq_i)  # len x 2 的邻接表
        self.seq_edge_index = self.seq_i.t()  # 转置 变为 2 * edgeSize   符合pyg输入规则
        self.seq_neighbors = torch.LongTensor(self.seq_neighbors)
        '''
            构建seq图结束  #############################################################
        '''
        ...
        print('user_id:{}, target_item:{}, target_item_date:{}, seq:{}'.format(self.user_id, self.target_item, self.target_item_date, self.seq))
    ...
    def _get_state(self):
        ...
        # 2.3 user 与 time  YELP: 年月日week ----> 四个维度 ----> 八条边
        ...
        state = {'cur_node': cur_node,      # acc_fea的集合(full_id)
                 ...
                 'seq_edge_index':self.seq_edge_index,
                 'seq_neighbors':self.seq_neighbors,        # full_id的节点集合
                 'seq_full_index_map_time_tuple': self.tuple_seq_item_time_list_node_map,
                 'seq_item_node_full_id_len' : self.seq_item_node_full_id_len,
                 'seq_fea_node_len':self.seq_fea_node_len,
                 }
        return state
    ...
    def step(self, action, sorted_actions, embed=None):
        ...
        if self.cur_conver_step == self.max_turn:
            ...
            # 回合增加惩罚
            reward = reward + self.reward_dict['turn_cont']
